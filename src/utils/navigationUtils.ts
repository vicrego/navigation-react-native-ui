...
export const calculationDistanceAndDuration = (
  currentLocation: any,
  ...
  const endPoint = turf.point(
    destinationRoute.coordinates[destinationRoute.coordinates.length - 1],
  );
  ...
  const remainingDuration =
    (remainingMiles / destinationDistance) * destinationDuration;
  return {
    remainingLine,
